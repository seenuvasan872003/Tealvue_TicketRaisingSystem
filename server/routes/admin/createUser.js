...
router.post('/create-user', ...guardRoute('create_user'), createAdminValidator, async (req, res) => {
  try {
    ...
    // Restrict standard admin to creating user and team_user only
    if (!['user', 'team_user'].includes(role)) {
      return res.status(403).json({ message: 'Admins can only create standard User or Team Agent accounts' });
    }
    ...
    await ActivityLog.create({
      ...
      note: `User account created with role ${role} for: ${user.name} (${user.email}) by admin: ${req.user.name}`
    });
